      ...
      {!loading && !error && (
        <div className="bg-white rounded-lg shadow overflow-hidden">
          <table className="w-full text-sm text-left">
            ...
            <tbody>
              {items.length === 0 && <tr><td colSpan={5} className="px-4 py-6 text-center text-gray-400">No portfolio items yet.</td></tr>}
              {items.map((p) => (
                <tr key={p.id} className="border-t">
                  ...
                  <td className="px-4 py-3 text-right space-x-3">
                    <Link href={`/admin/portfolio/${p.id}`} className="text-blue-600 hover:underline">Edit</Link>
