      ...
      {!loading && !error && items.length > 0 && (
        <>
          ...
          <div className="space-y-3 md:hidden">
            {items.map((t) => (
              <div key={t.id} onClick={() => router.push(`/admin/testimonials/${t.id}`)} className="cursor-pointer rounded-xl border border-gray-100 bg-white p-4 shadow-sm transition-all hover:border-[#0075ff]/30 hover:shadow-md hover:-translate-y-0.5">
                ...
                <div className="mt-3 flex items-center justify-between">
                  ...
                  <div className="space-x-3">
                    <Link href={`/admin/testimonials/${t.id}`} onClick={(e) => e.stopPropagation()} className="text-primary hover:text-accent transition-colors text-sm">Edit</Link>
                    <button onClick={(e) => { e.stopPropagation(); handleDelete(t.id, t.name); }} className="text-red-500 hover:text-red-700 transition-colors text-sm">Delete</button>
                  </div>
                </div>
              </div>
            ))}
          ...
          <div className="hidden md:block space-y-3">
            {items.map((t) => (
              <div key={t.id} onClick={() => router.push(`/admin/testimonials/${t.id}`)} className="flex cursor-pointer items-center gap-6 rounded-xl border border-gray-100 bg-white px-5 py-4 shadow-sm transition-all hover:border-[#0075ff]/30 hover:shadow-md hover:-translate-y-0.5">
                ...
                <div className="flex shrink-0 items-center gap-4 pl-2">
                  <Link href={`/admin/testimonials/${t.id}`} onClick={(e) => e.stopPropagation()} className="text-sm text-primary hover:text-accent transition-colors">Edit</Link>
                  <button onClick={(e) => { e.stopPropagation(); handleDelete(t.id, t.name); }} className="text-sm text-red-500 hover:text-red-700 transition-colors">Delete</button>
                </div>
              </div>
            ))}
          </div>
