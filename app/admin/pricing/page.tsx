...
export default function AdminPricingPage() {
  ...
  const handleDelete = async (id: string, name: string) => {
    ...
    try {
      const res = await adminFetch(`${process.env.NEXT_PUBLIC_API_URL}/pricing/${id}`, { method: 'DELETE', });
              ...
              <div key={p.id} onClick={() => router.push(`/admin/pricing/${p.id}`)} className="cursor-pointer rounded-xl border border-gray-100 bg-white p-4 shadow-sm transition-all hover:border-primary/20 hover:shadow-md">
                ...
                <div className="mt-3 flex items-center justify-between">
                  ...
                  <div className="space-x-3">
                    <Link href={`/admin/pricing/${p.id}`} onClick={(e) => e.stopPropagation()} className="text-primary hover:text-accent transition-colors text-sm">Edit</Link>
                    <button onClick={(e) => { e.stopPropagation(); handleDelete(p.id, p.name); }} className="text-red-500 hover:text-red-700 transition-colors text-sm">Delete</button>
                  </div>
                </div>
              </div>
            ))}
            ...
            {plans.map((p) => (
              <div key={p.id} onClick={() => router.push(`/admin/pricing/${p.id}`)} className="flex cursor-pointer items-center gap-6 rounded-xl border border-gray-100 bg-white px-5 py-4 shadow-sm transition-all hover:border-primary/20 hover:shadow-md">
                ...
                <div className="flex shrink-0 items-center gap-4 pl-2">
                  <Link href={`/admin/pricing/${p.id}`} onClick={(e) => e.stopPropagation()} className="text-sm text-primary hover:text-accent transition-colors">Edit</Link>
                  <button onClick={(e) => { e.stopPropagation(); handleDelete(p.id, p.name); }} className="text-sm text-red-500 hover:text-red-700 transition-colors">Delete</button>
                </div>
              </div>
            ))}
          </div>
